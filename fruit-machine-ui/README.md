...
支持英语、德语、西班牙语、俄语、葡萄牙语、越南语和中文。语言菜单显示各语言原名，保存玩家选择；未选择时使用支持的浏览器语言，SDK 模式可以接收主机的初始语言。切换语言保留下注、余额与当前收成。界面、规则、常见错误和清单均已本地化，金额与赔率沿用一致的小数表示。

翻译源文件在 `locales/*.json`。修改后运行 `python scripts/build-locales.py`（仓库根目录）校验字段和占位符并生成 `i18n.js`；浏览器只加载生成的静态文件。
