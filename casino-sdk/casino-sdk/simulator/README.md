...
# Fruit Tiger contract

`contracts/FruitTigerGame.sol` implements the conservative single-request VRF
version of the 24-cell ring game. `gameData` is ABI encoded as
`(uint8 version, uint8 mode, uint256[8] bets)`; version `1`, mode `0`, and the
sum of the eight bets must equal the session wager. `gameState` is encoded as
`(uint8 version, uint8 outcomeIndex, uint8 symbol, uint32 multiplierBps,
uint8 bonusType, bytes32 randomness)`. The fixed cell weights sum to 2,523 and
the paytable/weight profile is documented as the 94% RTP target configuration.
Lucky cells settle at zero in this first conservative implementation; bonus
expansion is intentionally deferred until its reserve model is specified.
