...
- 支持中文、英语、德语、西班牙语、俄语、葡萄牙语和越南语，适配桌面和手机。
...
支持英语、德语、西班牙语、俄语、葡萄牙语、越南语和中文，覆盖界面、玩法规则、结果消息、常见错误及 SDK 清单。玩家手动选择的语言会保存；未选择时使用支持的浏览器语言，嵌入主机可以提供初始语言。切换语言保留下注、余额与当前收成，金额和赔率使用一致的小数格式。

翻译源文件位于 `fruit-machine-ui/locales/*.json`，修改后在仓库根目录运行 `python scripts/build-locales.py`，检查字段和占位符并生成 `i18n.js`。`node scripts/graft-locales.test.cjs` 验证七种语言、刷新记忆、键盘操作和桌面/手机布局，需要 Playwright 与本地页面服务。
